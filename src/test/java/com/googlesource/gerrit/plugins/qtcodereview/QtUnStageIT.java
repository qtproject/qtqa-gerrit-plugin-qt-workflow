...
public class QtUnStageIT extends QtCodeReviewIT {
  ...
  private RevCommit qtUnStageExpectCherryPick(
      PushOneCommit.Result c, PushOneCommit.Result expectedContent) throws Exception {
    return qtUnStage(c, null, expectedContent, null, false, false);
  }

  private RevCommit qtUnStageExpectCherryPick(
      PushOneCommit.Result c, PushOneCommit.Result expectedContent, RevCommit base)
      throws Exception {
    return qtUnStage(c, null, expectedContent, base, false, false);
  }

  private RevCommit qtUnStageExpectCommit(PushOneCommit.Result c, RevCommit expectedStagingHead)
      throws Exception {
    return qtUnStage(c, expectedStagingHead, null, null, false, false);
  }

  private RevCommit qtUnStageExpectMergeFastForward(
      PushOneCommit.Result c, RevCommit expectedStagingHead) throws Exception {
    return qtUnStage(c, expectedStagingHead, null, null, true, true);
  }

  private RevCommit qtUnStageExpectMergeOfMerge(
      PushOneCommit.Result c, PushOneCommit.Result expectedContent) throws Exception {
    return qtUnStage(c, null, expectedContent, null, true, false);
  }

  private RevCommit qtUnStage(
