...
public class QtReOpenIT extends QtCodeReviewIT {
  ...
  private RevCommit qtReOpen(PushOneCommit.Result c, RevCommit initialHead) throws Exception {
  ...
  private RestResponse qtReOpenExpectFail(PushOneCommit.Result c, int expectedStatus)
      throws Exception {
    RestResponse response = call_REST_API_ReOpen(c.getChangeId());
